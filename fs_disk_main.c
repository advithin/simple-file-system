 ...
 */
 ...
 /*
  export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/lib/x86_64-linux-gnu/
 	gcc -Wall -D_FILE_OFFSET_BITS=64 fs_disk_main1.c `pkg-config fuse --cflags --libs` -o exe
 	./exe -f mount
...
int main(int argc, char *argv[])
{
  ...
  printf("Welcome!!\n\n");
	...
	printf("root_directory = %u\n", root_directory);
  	if(buf.st_size == 0)
  	{
  		strcpy(root_directory -> filename, "AaksAayuAddu");
		...
		temp = inodes;
		printf("inode of root directory\n");
		print_inode(temp);
		printf("\n");
...
void allocate_inode(char *path, int *ino, bool dir){
	...
	printf("inode address to allocate : %u   address of ino:%u  beg of inodes:%u\n", *ino,temp_ino,inodes);
  ...
  if(dir){
  printf("allocating 2 links for a directory\n");
    temp_ino -> link_count = 2;
  }
  else{
    temp_ino -> link_count = 1;
  }
  printf("ino :%d\n  ttemp_ino -> directory : %d\n",*ino,temp_ino -> directory );
 ...
 */
  		       ...
  		       struct fuse_file_info *fi)
  {
	...
	inode * inode_addr;
  	...
  	if (directory_flag == 1) {
  		...
  		inode_addr=inodes + ((ino) * sizeof(inode));
  		stbuf->st_nlink =inode_addr-> link_count;
  		//stbuf->st_nlink = 2;
  	}
  ...
  static int fs_mkdir(const char *path, mode_t mode)
  {
  	...
  	allocate_inode(path, &ino, true);
    	inode *temp_ino = inodes + (ino * sizeof(inode));
    	printf("ino : %u   address of ino:%u  beg of inodes:%u\n", ino,temp_ino,inodes);
    printf("printing inode\n why is directory 0?\n");
  	...
  	while (token != NULL)
  	{
  		...
  		if((strcmp(temp -> filename, "") == 0))
  		{
  			...
  			printf("inside mkdir checking link count\n");
  			printf("%d\n",temp_ino->link_count);
			temp_ino->link_count++;
			printf("%d\n",temp_ino->link_count);
  			temp -> file_inode = ino;

        		printf("\n\nPersistinggg the directory creation\n\n");
